...
export default function CongViecModal({ congViecUp, open, setOpen, reloadList }) {
    ...
    function loadSelect() {
        setLoading(true)
        Services.getCongViecService().getChiTietCongViec(congViecUp.id).then(
            (response) => {
                if (isMounted) {
    ...
    const handleSubmit = () => {
        let err = congViecUtil.kiemTaCongViecCapNhat(congViec)
        if (err == "") {
            Services.getCongViecService().capNhat(congViec).then(
                (response) => {
                            ...
                            <Grid container spacing={6} >
                                <Grid item lg={12} md={12} sm={12} xs={12} sx={{ m: 2 }}>
                                    <Autocomplete
                                        ...
                                        value={[congViec.chiBo]}
                                        options={[congViec.chiBo]}
