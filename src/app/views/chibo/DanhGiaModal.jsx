...
import danhGiaXepLoaiCBUtil from 'app/utils/modules/DanhGiaXepLoaiCB';
...
export default function DanhGiaModal({ listChiBo, xepLoaiUp, open, setOpen, reloadList, namXepLoai }) {

    const [xepLoai, setXepLoai] = useState(danhGiaXepLoaiCBUtil.getDanhGiaXepLoaiCB());
    ...
    function loadSelect() {
        ...
        } else setXepLoai(danhGiaXepLoaiCBUtil.getDanhGiaXepLoaiCB());
    }
    const handleThem = () => {
        let err = danhGiaXepLoaiCBUtil.kiemTaDanhGiaXepLoaiCB(xepLoai)
        if (err == "") {
            let listXL = []
            listChiBo.forEach(dv => {
                listXL.push({ ...xepLoai, nam: namXepLoai, chiBo: dv })
            });
            // console.log(listXL);
            Services.getDanhGiaCBService().themList(listXL).then(
                (response) => {
    ...
    const handleCapNhat = () => {
        let err = danhGiaXepLoaiCBUtil.kiemTaDanhGiaXepLoaiCB(xepLoai)
        if (err == "") {
            Services.getDanhGiaCBService().capNhat({ ...xepLoai, chiBo: listChiBo[0] }).then(
                (response) => {
                    if (response.data?.tenLoi != undefined) {
                        setError(response.data?.noiDung)
                    } else {
    ...
    const handleClose = () => { setOpen(false); setError(""); setXepLoai(danhGiaXepLoaiCBUtil.getDanhGiaXepLoaiCB()) };

    return (
        <>
            <Modal size="sm" backdrop="static" keyboard={false} open={open} onClose={handleClose} className="cus-modal">
                <Modal.Header>
                    <Modal.Title><b>{"KẾT QUẢ ĐÁNH GIÁ CHẤT LƯỢNG CHI BỘ NĂM "}<span className='red'>{namXepLoai}</span></b></Modal.Title>
                </Modal.Header>
                        ...
                        <Grid container spacing={6} >
                            <Grid item lg={12} md={12} sm={12} xs={12} sx={{ m: 2 }}>
                                <Stack style={{ marginBottom: "13px" }}>
                                    <p><b>{listChiBo.map(item => item.ten + ", ")}</b></p>
                                </Stack>
                                <Autocomplete
