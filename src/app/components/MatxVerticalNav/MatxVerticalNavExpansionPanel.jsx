  ...
  return (
    <NavExpandRoot>
      <BaseButton
        ...
        onClick={handleClick}
      >
        <Box display="flex" alignItems="center" sx={{ width: 170 }}>
          {icon && <Icon className="icon">{icon}</Icon>}
          {iconText && <BulletIcon />}
          <ItemText className="sidenavHoverShow">{name}</ItemText>
        </Box>

        {badge?.value && <BadgeValue className="sidenavHoverShow itemIcon icon-count" sx={{ color: "red" }}>{badge.value}</BadgeValue>}
